...
$sql = "SELECT sum(money) FROM ckinout WHERE ((type='1') or (type = '3') or (type='2' and specialtype='1')) and (date <= '" . $ld . "')";
$result = mysqli_query($connection, $sql);
$row = $result->fetch_array(MYSQLI_NUM);
$outcome = (int)$row[0];

$sql = "SELECT sum(money) FROM ckinout WHERE (type='2') and (date <= '" . $ld . "')";
$result = mysqli_query($connection, $sql);
$row = $result->fetch_array(MYSQLI_NUM);
$debt = (int)$row[0];

$sql = "SELECT sum(money) FROM ckinout WHERE (type='3') and (date <= '" . $ld . "')";
$result = mysqli_query($connection, $sql);
$row = $result->fetch_array(MYSQLI_NUM);
$paydebt = (int)$row[0];
